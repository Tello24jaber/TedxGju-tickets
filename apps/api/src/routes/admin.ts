...
router.post('/requests/:id/approve', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('Tickets sent successfully via email');
    } catch (emailError: any) {
      console.error('Failed to send tickets via email:', emailError.message);
      // Don't fail the approval if email fails - log it and continue
      // Tickets are created and can be resent later
    }
